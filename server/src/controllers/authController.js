...
async function requestPasswordReset(req, res) {
  ...
  try {
    ...
    if (user && user.is_active && user.role !== ROLES.SUPER_ADMIN) {
      ...
      const { orgName, orgLogoUrl } = getAppSettings();
      const { subject, html } = templates.passwordReset({ fullName: user.full_name, resetUrl, orgName: orgName || '', orgLogoUrl: orgLogoUrl || '' });
