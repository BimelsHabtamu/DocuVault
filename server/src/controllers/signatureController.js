...
const { computeSignatureHmac, getApproverSecret } = require('../utils/hmac');
...
async function applyApproval(sigReq) {
  ...
  const { timestamp, source: timeSource } = await getSyncedTime();
  if (timeSource !== 'ntp') {
    console.warn(`[signature] applyApproval: signing timestamp for doc ${sigReq.doc_id} used ${timeSource} — NTP was unavailable.`);
  }
  ...
  // FR-024: per-approver signing secret — never the server-wide fallback for new sigs.
  const approverSecret = await getApproverSecret(approver.id);
  const cryptoHmac = computeSignatureHmac(newHash, timestamp.toISOString(), approverSecret);
  ...
  await pool.query('UPDATE generated_docs SET status = ?, file_hash = ?, metadata = ? WHERE id = ?', [
    'signed', newHash, JSON.stringify({ ...meta, signedAt: timestamp.toISOString(), signedTimestampSource: timeSource }), doc.id,
  ]);
