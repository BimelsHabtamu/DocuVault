...
async function buildRenderedDocument(template, recordId) {
  ...
  // FR-013 / M-1: auto-filled date placeholders injected before rendering.
  // generation_date / _gc / _ec — always the PDF creation date.
  // effective_date / _gc / _ec  — defaults to generation_date; overridden by
  //   source-record data if the mapped table has an effective_date column.
  const automaticDateHtml = `
        <div class="document-dates">
            ...
            <p><strong>Effective Date:</strong> ${dataContext.effective_date_gc} &nbsp;/&nbsp; ${dataContext.effective_date_ec}</p>
        </div>
    `;
...
async function generateSingleDocument({ template, recordId, userId }) {
  ...
  // actually sees. It does NOT cover the QR image itself or PDF rendering artefacts.
  // See documentIntegrity.js for the full explanation.
  const contentHtml = `${headerHtml}${bodyHtml}${automaticDateHtml}${footerHtml}`;
  const contentHash = sha256(Buffer.from(contentHtml, 'utf8'));

  const verificationId = generateVerificationId(); // Secure Delivery module: opaque public QR identifier
  // Pass contentHash + issuedAt so the QR payload includes all SRS-required fields:
  // Doc ID, issuedAt timestamp, SHA-256 content hash, and verify URL (FR-016).
  const hashVerifyFooterHtml = await buildTamperProofFooterHtml(docId, VERIFY_BASE_URL, contentHash, issuedAt);
  ...
  const fileHash = sha256(pdfBuffer); // FR-016, NFR-003: final PDF hash stored in DB for file-level verify
  ...
  const [result] = await pool.query(
    ...
    [docId, verificationId, template.id, userId, String(recordId), storagePath, fileHash, JSON.stringify({
      fileName,
      contentHash, // FR-016: stored alongside fileHash so /verify can report both
  ...
  return { id: result.insertId, docUuid: docId, fileName, fileHash, contentHash, sizeBytes: pdfBuffer.length };
}
...
async function downloadDocument(req, res) {
  ...
  try {
    ...
    // C-1/P-7: SRS RBAC download matrix
    //   super_admin / system_admin — can download any document (admin oversight)
    //   approver      — can download any signed document (they approved/reviewed it per SRS)
    //   generator     — can only download documents THEY generated (own-docs only)
    //   recipient     — served via the secure-delivery OTP flow, never this endpoint
    const { role, id: userId } = req.user;
    if (role === 'recipient') {
      return res.status(403).json({ success: false, message: 'Recipients access documents through the secure-delivery link sent by email.' });
    }
    if (role === 'generator' && doc.generated_by !== userId) {
      return res.status(403).json({ success: false, message: 'You can only download documents you generated.' });
    }
    if (role === 'approver' && doc.status === 'draft') {
      // Approvers use the signature-request review route for pending documents.
      // They may download any signed/delivered/rejected document they acted upon.
      return res.status(403).json({ success: false, message: 'Draft documents are not yet approved — view pending documents from your approvals queue.' });
    }
  ...
  try {
    ...
    if (doc.notify_view_token_used_at) {
      return res.status(410).json({
        success: false,
        message: 'This one-time review link has already been used. Please sign in to DocuVault and open it from My Documents instead.',
      });
    }
    if (doc.deleted_at) {
