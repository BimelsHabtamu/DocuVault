 ...
 * Dashboard KPIs scoped to the currently logged-in user — safe for Generator and
 * Approver roles to call (does NOT expose system-wide data).
 *
 * Returns:
 *   - totalDocs        — total docs this user has generated
 *   - generatedToday   — docs generated by this user today
 *   - pendingApproval  — docs from this user currently pending (signature_status = 'pending')
 *   - approved         — docs from this user that are signed/delivered
 *   - rejected         — docs from this user that are rejected (and not resubmitted)
 *   - pendingToReview  — for approver: docs currently pending in their approval queue
 *   - recentDocs       — last 5 docs this user generated (id, doc_uuid, template_name,
 *                        status, generated_at, signature_status)
 *   - recentActivity   — last 10 audit events touching this user's docs
 *   - pendingQueue     — for approver: up to 5 pending requests for them to review
 */
async function getMyDashboardStats(req, res) {
  const userId = req.user.id;
  const role = req.user.role;
  const isApprover = role === 'approver' || role === 'super_admin' || role === 'system_admin';

  try {
    // --- Core counts scoped to this user's generated docs ---
    const [[{ totalDocs }]] = await pool.query(
      `SELECT COUNT(*) AS totalDocs FROM generated_docs WHERE generated_by = ?`,
      [userId]
    );

    const [[{ generatedToday }]] = await pool.query(
      `SELECT COUNT(*) AS generatedToday FROM generated_docs
       WHERE generated_by = ? AND DATE(generated_at) = CURDATE()`,
      [userId]
    );

    // "pending" = has a live pending signature_request
    const [[{ pendingApproval }]] = await pool.query(
      `SELECT COUNT(*) AS pendingApproval
       FROM generated_docs gd
       WHERE gd.generated_by = ?
         AND EXISTS (
           SELECT 1 FROM signature_requests sr
           WHERE sr.doc_id = gd.id AND sr.status = 'pending'
         )`,
      [userId]
    );

    // "approved" = doc status is signed OR delivered
    const [[{ approved }]] = await pool.query(
      `SELECT COUNT(*) AS approved FROM generated_docs
       WHERE generated_by = ? AND status IN ('signed', 'delivered')`,
      [userId]
    );

    // "rejected" = latest signature_request for this doc is rejected
    const [[{ rejected }]] = await pool.query(
      `SELECT COUNT(*) AS rejected
       ...
       WHERE gd.generated_by = ?
         AND EXISTS (
           SELECT 1 FROM signature_requests sr
           WHERE sr.doc_id = gd.id AND sr.status = 'rejected'
             AND sr.id = (
               SELECT sr2.id FROM signature_requests sr2
               WHERE sr2.doc_id = gd.id ORDER BY sr2.created_at DESC LIMIT 1
             )
         )`,
      [userId]
    );

    // For approver roles: how many items are in their approval queue right now
    let pendingToReview = 0;
    if (isApprover) {
    ...
    // --- Recent 5 documents this user generated ---
    const [recentDocs] = await pool.query(
      `SELECT gd.id, gd.doc_uuid, gd.status, gd.generated_at, gd.record_identifier,
              t.name AS template_name,
              latest_sr.status AS signature_status,
              latest_sr.rejection_reason
       FROM generated_docs gd
       JOIN templates t ON t.id = gd.template_id
       LEFT JOIN signature_requests latest_sr
         ON latest_sr.id = (
    ...
    // --- Recent 10 audit events touching this user's docs ---
    const [recentActivity] = await pool.query(
      `SELECT al.id, al.action, al.timestamp, al.action_details,
              gd.doc_uuid, t.name AS template_name,
    ...
    // --- For approver: pending queue (up to 5 most recent) ---
    let pendingQueue = [];
    if (isApprover) {
      const [queueRows] = await pool.query(
        `SELECT sr.id, sr.doc_id, sr.created_at, gd.doc_uuid, gd.record_identifier,
                t.name AS template_name, gen_u.full_name AS generator_name
         FROM signature_requests sr
         JOIN generated_docs gd ON gd.id = sr.doc_id
         JOIN templates t ON t.id = gd.template_id
         JOIN users gen_u ON gen_u.id = gd.generated_by
    ...
    return res.status(200).json({
      success: true,
      message: 'My dashboard stats fetched.',
      data: {
        totalDocs,
        generatedToday,
        pendingApproval,
    ...
    console.error('[dashboard] my-stats error:', err);
