...
/** PUT /api/settings   body: { escalationHours?, archiveYears?, minutesSavedPerDoc?, orgName?, orgLogoUrl? } */
async function putSettings(req, res) {
  const { escalationHours, archiveYears, minutesSavedPerDoc, orgName, orgLogoUrl } = req.body;
  const partial = {};
  ...
  // Enterprise branding for transactional emails (org name fallback + org logo).
  const stringOrNull = (v) => (typeof v === 'string' ? v.trim() : '');
  if (orgName !== undefined) partial.orgName = stringOrNull(orgName);
  if (orgLogoUrl !== undefined) partial.orgLogoUrl = stringOrNull(orgLogoUrl);
