...
async function getNotifications(req, res) {
  try {
    ...
    // Admin delivery-completion feed: super_admin and system_admin see a
    // notification whenever ANY document is fully delivered (recipient confirmed
    // ownership = DELIVERY_OWNED_NOTIFY) OR when a recipient rejects ownership
    // (OWNERSHIP_REJECTED_NOTIFY). Admins already see rejected-approval events
    // above; this fills the gap for the delivery side of the lifecycle.
    // Skipped entirely for non-admin roles to keep the query cost zero.
    let adminDeliveryEvents = [];
    if (req.user.role === 'super_admin' || req.user.role === 'system_admin') {
      [adminDeliveryEvents] = await pool.query(
        `SELECT al.id, al.timestamp, al.action, al.action_details,
                gd.doc_uuid, gd.id AS doc_id, gd.file_path, gd.generated_by,
                NULL AS signature_request_id,
                CASE
                  WHEN al.action = 'DELIVERY_OWNED_NOTIFY'     THEN 'admin_delivery_confirmed'
                  WHEN al.action = 'OWNERSHIP_REJECTED_NOTIFY' THEN 'admin_ownership_rejected'
                  ELSE 'admin_delivery_event'
                END AS notification_type
         FROM audit_logs al
         JOIN generated_docs gd ON gd.id = al.doc_id
         JOIN users gen ON gen.id = gd.generated_by
         WHERE al.action IN ('DELIVERY_OWNED_NOTIFY', 'OWNERSHIP_REJECTED_NOTIFY')
           AND gen.role NOT IN ('super_admin', 'system_admin')
         ORDER BY al.timestamp DESC
         LIMIT 20`
      );
    }
    ...
    const combined = [...approverEvents, ...generatorEvents, ...ownershipRejectedEvents, ...ownedEvents, ...adminDeliveryEvents]
