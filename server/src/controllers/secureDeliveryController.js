...
/**
 * GET /api/documents/:id/recipient-email
 * Looks up the email address stored on the data-source record that was used
 * to generate this document, so the Send modal can pre-fill it automatically.
 * The generator still confirms before sending — this is just a convenience lookup.
 * Returns { email, recipientName } or { email: null } when no email column exists.
 */
async function getRecipientEmailForDoc(req, res) {
  const { id } = req.params;
  try {
    const [[doc]] = await pool.query(
      `SELECT gd.record_identifier, gd.recipient_email,
              t.data_source_table, t.data_source_connection_id
       FROM generated_docs gd
       JOIN templates t ON t.id = gd.template_id
       WHERE gd.id = ? AND gd.generated_by = ?`,
      [id, req.user.id]
    );
    if (!doc) {
      return res.status(404).json({ success: false, message: 'Document not found.' });
    ...
    // If the document was already delivered, use the stored recipient email directly
    if (doc.recipient_email) {
      return res.status(200).json({
        success: true,
        data: { email: doc.recipient_email, source: 'previous_delivery' },
      });
    }

    if (!doc.data_source_table || !doc.record_identifier) {
      return res.status(200).json({ success: true, data: { email: null } });
    }

    // Fetch the email from the data source record
    let emailValue = null;
    let nameValue = null;

    if (doc.data_source_connection_id) {
      // External connection
      const { loadConnectionConfig } = require('./externalDbController');
      const { fetchRecordById } = require('../utils/externalDbClients');
      const config = await loadConnectionConfig(doc.data_source_connection_id);
      if (config) {
        const record = await fetchRecordById(config, doc.data_source_table, doc.record_identifier);
        if (record) {
          const keys = Object.keys(record);
          const emailCol = keys.find((k) => /email/i.test(k));
          const nameCol  = keys.find((k) => /^(full_?name|name|first_?name)$/i.test(k));
          emailValue = emailCol ? String(record[emailCol] || '').trim() : null;
          nameValue  = nameCol  ? String(record[nameCol]  || '').trim() : null;
        }
      }
    } else {
      // Internal table — discover email/name columns then fetch the record
      const [cols] = await pool.query(
        `SELECT COLUMN_NAME FROM information_schema.COLUMNS
         WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?`,
        [process.env.DB_NAME || 'doc_automation', doc.data_source_table]
      );
      const colNames = cols.map((c) => c.COLUMN_NAME);
      const emailCol = colNames.find((c) => /email/i.test(c));
      const nameCol  = colNames.find((c) => /^(full_?name|name|first_?name)$/i.test(c));

      if (emailCol) {
        const singularKey = `${doc.data_source_table.replace(/s$/, '')}_id`;
        const lookupCol   = colNames.includes(singularKey) ? singularKey : 'id';
        const [[row]] = await pool.query(
          `SELECT \`${emailCol}\`${nameCol ? `, \`${nameCol}\`` : ''} FROM \`${doc.data_source_table}\`
           WHERE \`${lookupCol}\` = ? LIMIT 1`,
          [doc.record_identifier]
        );
        if (row) {
          emailValue = String(row[emailCol] || '').trim() || null;
          nameValue  = nameCol && row[nameCol] ? String(row[nameCol]).trim() : null;
        }
      }
    }
    ...
    return res.status(200).json({
      success: true,
      data: { email: emailValue || null, recipientName: nameValue || null, source: 'data_source' },
    });
  } catch (err) {
    console.error('[secureDelivery] getRecipientEmailForDoc error:', err.message);
    return res.status(500).json({ success: false, message: 'Failed to look up recipient email.' });
  }
}

module.exports = {
  getRecipientEmailForDoc,
