...
const { verifyDocument, verifyByDocUuid, verifyByUpload, verifyDocumentSignature } = require('../controllers/verifyController');
const {
  viewDocumentByToken,
...
// The unified endpoint handles three modes via a custom multer-or-json flow:
//   • multipart with a PDF file  → file-hash verification
//   • JSON { doc_id }            → Doc ID look-up
//   • JSON { qr_payload }        → FR-016 QR payload verification (Phase 1 structured QR)
// multer.single('pdf') is called here; if the body is JSON (no file), multer is a
// no-op and req.body is populated normally by express.json upstream.
router.post('/verify', upload.single('pdf'), verifyDocument);

// FR-024: Digital signature HMAC verification — public, no login required.
// GET /api/verify/:doc_uuid/signature  → verifies the HMAC on the stored signature.
// Must be registered BEFORE the legacy GET /:doc_uuid route in verifyRoutes to avoid
// being shadowed (publicRoutes owns this path; verifyRoutes owns the legacy one).
router.get('/verify/:doc_uuid/signature', verifyDocumentSignature);
