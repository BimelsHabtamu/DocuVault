...
async function ensureSchema() {
  ...
  // 7) users.organization — free-text org name typed by the admin when creating an
  //    account (migration 024). Powers the "You've been invited to join <org> on
  //    DocuVault" line + account-details block in the welcome email. Idempotent on boot.
  try {
    const [cols] = await pool.query(
      `SELECT COLUMN_NAME FROM information_schema.COLUMNS
       WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'organization'`,
      [dbName]
    );
    if (cols.length === 0) {
      await pool.query(`ALTER TABLE users ADD COLUMN organization VARCHAR(255) NULL AFTER phone`);
      console.log('[db] Added organization to users.');
    }
  } catch (err) {
    console.error('[db] Could not ensure users.organization exists:', err.message);
  }

  // FR-024: per-approver HMAC signing secret, stored AES-256-GCM encrypted.
  // Provisioned on first signing call (getApproverSecret in hmac.js), never
  // set to a plaintext value here.
  try {
    const [cols] = await pool.query(
      `SELECT COLUMN_NAME FROM information_schema.COLUMNS
       WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'signing_secret_enc'`,
      [dbName]
    );
    if (cols.length === 0) {
      await pool.query(`ALTER TABLE users ADD COLUMN signing_secret_enc TEXT NULL AFTER organization`);
      console.log('[db] Added signing_secret_enc to users (FR-024 per-approver signing secret).');
    }
  } catch (err) {
    console.error('[db] Could not ensure users.signing_secret_enc exists:', err.message);
  }
