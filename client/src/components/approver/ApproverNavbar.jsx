...
function getTitle(pathname) {
  ...
  // Partial prefix match (e.g. /approvals?tab=pending still matches /approvals)
...
function SearchBar() {
  ...
  const handleSubmit = e => {
    e.preventDefault();
    if (value.trim()) navigate(`/documents?search=${encodeURIComponent(value.trim())}`);
  };

  return (
    <form onSubmit={handleSubmit}
      className={`relative transition-all duration-200
        ${focused ? 'w-52 sm:w-72' : 'w-36 sm:w-52'}`}>
      ...
      <input
        ...
        style={{
          borderColor: focused ? '#7c3aed' : '#e5e7eb',
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-xl
            ...
            {items.map(n => (
              <button key={n.id}
                ...
                className={`flex items-start gap-3 w-full px-4 py-3 text-left
                  ...
                  ${n.unread ? 'bg-violet-50/30' : ''}`}>
                <div className="w-2 h-2 rounded-full mt-1.5 flex-shrink-0"
                  style={{ backgroundColor: '#7c3aed', opacity: n.unread ? 1 : 0 }} />
          ...
          <div className="border-t border-gray-100 px-4 py-2.5">
            <button onClick={() => { setOpen(false); navigate('/approvals'); }}
              className="text-xs font-semibold w-full text-center text-violet-600
...
function LangToggle() {
  ...
  return (
    <button onClick={() => i18n.changeLanguage(isAm ? 'en' : 'am')}
    ...
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(o => !o)}
        ...
        <div className="text-left hidden sm:block">
          ...
          <p className="text-[11px] text-gray-500 leading-tight capitalize">
        ...
        <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl
          ...
          <div className="px-4 py-3 border-b border-gray-100">
            ...
            <span className="inline-block mt-1.5 text-[10px] font-semibold
              px-2 py-0.5 rounded-full bg-violet-50 text-violet-700 capitalize">
              {user?.role?.replace(/_/g, ' ')}
            </span>
          </div>
...
// ── Navbar root ───────────────────────────────────────────────────────────────
export default function ApproverNavbar({ onMenuClick }) {
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-200 flex items-center
      ...
      {/* Left: hamburger · title · divider · search */}
