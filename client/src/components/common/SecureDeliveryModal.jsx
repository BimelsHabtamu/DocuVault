...
export default function SecureDeliveryModal({ doc, onClose, onSent }) {
  ...
  const [emailLookupStatus, setEmailLookupStatus] = useState('loading'); // 'loading' | 'found' | 'not_found' | 'error'

  // Auto-fetch the recipient email from the data source record on mount
  useEffect(() => {
    setEmailLookupStatus('loading');
    deliveryService.getRecipientEmail(doc.id)
      .then((res) => {
      ...
      .catch(() => setEmailLookupStatus('error'));
  }, [doc.id]);
          ...
          {sentResult ? (
            ...
            <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 6 }}>
                  {METHODS.map((m) => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ flexShrink: 0, marginTop: 2, color: selected ? 'var(--brand)' : 'var(--text-secondary)' }}>
                          <Icon size={18} color={selected ? 'var(--brand)' : 'var(--text-secondary)'} />
                        </span>
                        <span>
                          <span style={{ display: 'block', fontWeight: 600, fontSize: '0.9rem', color: selected ? 'var(--brand-text)' : 'var(--text-primary)' }}>
                            {m.label}{selected && (
                              <span style={{ marginLeft: 6, display: 'inline-flex', verticalAlign: 'middle' }}>
                                <CheckIcon size={14} color="var(--brand)" />
                              </span>
                            )}
                          </span>
              ...
              <div className="form-field" style={{ marginTop: 12 }}>
                ...
                {emailLookupStatus === 'loading' && (
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 5 }}>
                    <span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', border: '2px solid var(--accent)', borderTopColor: 'transparent', animation: 'spin 0.6s linear infinite' }} />
                    Looking up recipient email from record {recordId}…
                  </div>
                )}
                {emailLookupStatus === 'found' && (
                  <div style={{ fontSize: '0.78rem', color: '#0F766E', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 5 }}>
                    <CheckIcon size={13} color="#0F766E" /> Email automatically filled from record <b>{recordId}</b> — confirm it is correct before sending.
                  </div>
                )}
                {emailLookupStatus === 'not_found' && (
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 4 }}>
                    No email found on record <b>{recordId}</b> — enter the recipient email manually.
                  </div>
                )}
                {emailLookupStatus === 'error' && (
                  <div style={{ fontSize: '0.78rem', color: 'var(--warn-text)', marginBottom: 4 }}>
                    Could not look up record — enter the recipient email manually.
                  </div>
                )}
                <input
                  ...
                  autoComplete="off"
                  style={emailLookupStatus === 'found' ? { borderColor: 'var(--accent)', background: 'var(--accent-light)' } : {}}
                />
              </div>
              ...
              {trimmedEmail && emailLooksValid && !confirming && (
                <div className="send-doc-field" style={{ marginTop: 14 }}>
                  ...
                  {method === 'secure_link_otp' ? (
                    <>
                      <div className="send-doc-radio-desc" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <LockIcon size={14} color="var(--brand)" /> Secure link + OTP → OWN → Download
                      </div>
                      <div className="send-doc-radio-desc" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
