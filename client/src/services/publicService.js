...
export async function verifyByQrPayload(qrPayloadString) {
  const res = await fetch(`${BASE_URL}/verify`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ qr_payload: qrPayloadString }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || 'Verification failed.');
  return data;
...
/** verify by uploading the PDF itself (multipart, not JSON). */
...
export async function verifyDocumentSignature(docId) {
  const res = await fetch(`${BASE_URL}/verify/${encodeURIComponent(docId)}/signature`);
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || 'Signature verification failed.');
  return data;
}
